...
def log_likelihood(theta,connection=False,returnall=False,pid=0, genpdf_only=False):               
    ...
    try:                   
        if connection == False: #For MCMC running, will pick up a connection              
            sys.stdout.flush()                                                            
            connection = connections[(current_process-1)]#formerly connections[(current_process()._identity[0]-1) % len(connections)]
        ...
        try: 
            if np.isnan(connection.beta):                                                 
                ...
                newcon = (current_process-1) #% see above at original connection generator, this has been changed
...
#    with Pool(npool) as pool:
    sampler = nautilus.Sampler(prior, log_probability, n_live=nlive, n_dim=ndim, pass_dict=False,  filepath=fileout, resume=resume, seed=seed)
