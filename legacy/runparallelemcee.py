...
tempin = [
    ...
    1.85968678,
    0.32786696,
]  # 4th best from 10/2 fit, starts at -39 and gives -72.8
...
tempin = [
    ...
    2.01780004,
    0.34809447,
]  # 5th best from 10/2 fit, starts at -39 and gives -64
...
paramdict = {
    "c": ["c_m", "c_std"],
    "x1": ["x1_m", "x1_l", "x1_r"],
    "EBV": ["EBV_Tau_low", "EBV_Tau_high"],
    "RV": ["RV_m_low", "RV_std_low", "RV_m_high", "RV_std_high"],
    "beta": ["beta_m", "beta_std"],
    "EBVZ": ["EBVZL_Tau_low", "EBVZL_Tau_high", "EBVZH_Tau_low", "EBVZH_Tau_high"],
}
...
cleandict = {
    "c_m": [-0.08, 0.01, False, [-0.3, -0.04]],
    "c_l": [0.044, 0.03, True, [0.01, 0.2]],
    "c_r": [0.044, 0.03, True, [0.01, 0.2]],
    "c_std": [0.044, 0.02, True, [0.01, 0.2]],
    "x1_m": [0, 0.05, False, [-2, 2]],
    "x1_l": [1, 1, True, [0.01, 2]],
    "x1_r": [1, 1, True, [0.01, 2]],
    "EBV_Tau_low": [0.11, 0.02, True, [0.08, 0.2]],
    "EBV_Tau_high": [0.13, 0.02, True, [0.08, 0.2]],
    "RV_m_low": [2, 0.5, True, [0.8, 4]],
    "RV_l_low": [1, 0.5, True, [0.1, 4]],
    "RV_r_low": [1, 0.5, True, [0.1, 4]],
    "RV_std_low": [1.5, 0.5, True, [0.25, 4]],
    "RV_m_high": [2, 0.5, True, [0.8, 4]],
    "RV_l_high": [1, 0.5, True, [0.1, 4]],
    "RV_r_high": [1, 0.5, True, [0.1, 4]],
    "RV_std_high": [1.5, 0.5, True, [0.25, 4]],
    "beta_m": [2, 0.5, False, [1, 3]],
    "beta_std": [0.2, 0.07, True, [0, 1]],
    "EBVZL_Tau_low": [0.11, 0.02, True, [0.05, 0.2]],
    "EBVZL_Tau_high": [0.13, 0.02, True, [0.05, 0.2]],
    "EBVZH_Tau_low": [0.11, 0.02, True, [0.05, 0.2]],
    "EBVZH_Tau_high": [0.13, 0.02, True, [0.05, 0.2]],
    "EBV_mu_low": [],
    "EBV_std_low": [],
    "EBV_mu_high": [],
    "EBV_std_high": [],
...
def input_cleaner(
    inp_params, cleandict, override
):  # this function takes in the input parameters and generates the walkers with appropriate dimensions, starting points, walkers, and step size
...
def pconv(
    inp_params,
):  # converts simple input list of parameters into the expanded list that characterises the sample
    inpfull = []
    for i in inp_params:
...
def Criteria_Plotter(theta):
    ...
    try:
        chisq, datacount_list, simcount_list, poisson_list = log_likelihood(
            (theta), returnall=True, connection=tc
        )
    except TypeError:
        print(
    ...
    ax = axs[1]
    ax.errorbar(
        cbins, datacount_list[1], yerr=(poisson_list[1]), fmt="^", c="k", label="Data, High Mass"
    )
    ...
    ax = axs[2]
    ax.errorbar(
        cbins, datacount_list[3], yerr=(poisson_list[3]), fmt="^", c="k", label="REAL DATA HIMASS"
    )
    ...
    plt.savefig(
        "figures/" + data_input.split(".")[0] + "overplot_observed_DATA_SIM_OVERVIEW.pdf",
        pad_inches=0.01,
        bbox_inches="tight",
    )
...
def init_connection(index, real=True, debug=False):
    ...
    if real:
        ...
        if debug:
            cmd = cmd + " SUBPROCESS_OPTMASK={OPTMASK} "
        realdata = callSALT2mu.SALT2mu(
            cmd, "NOTHING.DAT", realdataout, subprocess_log_data, realdata=True, debug=debug
        )
    else:
        realdata = 0
    cmd = (
...
carr = np.arange(-0.5, 0.5, 0.001)  # these are fixed so can be defined globally
    ...
    try:
        ...
        for inp in inp_params:  # TODO - need to generalise to 2d functions as well
            if "RV" in inp:
                ...
                connection.write_2D_Mass_PDF_SYMMETRIC(
                    simdic[inp], thetawriter(theta, inp), arrdic[inp]
                )
            elif "EBV" in inp:
                if splitEBV:  # need to hold here to pass off both EBVZH and EBVZL simultaneously
                    if len(thetawriter(theta, inp)) == 4:
                        ...
                        connection.write_3D_MassEBV_PDF(
                            simdic[inp], thetawriter(theta, inp), arrdic[inp]
                        )
                    else:
                        print("Using LOGNORMAL EBV model", flush=True)
                        connection.write_3D_LOGNORMAL_PDF(
                            simdic[inp], thetawriter(theta, inp), arrdic[inp]
                        )
                else:
                    if len(thetawriter(theta, inp)) == 2:
                        print("Using Exponential EBV model", flush=True)
                        connection.write_2D_MassEBV_PDF(
                            simdic[inp], thetawriter(theta, inp), arrdic[inp]
                        )
                    else:
                        print("Using LOGNORMAL EBV model", flush=True)
                        connection.write_2D_LOGNORMAL_PDF(
                            simdic[inp], thetawriter(theta, inp), arrdic[inp]
                        )
...
def log_prior(
    theta, debug=False
):  # goes through expanded input parameters and checks that they are all within range. If any are not, returns negative infinity.
    thetadict = thetaconverter(theta)
    plist = pconv(inp_params)
    if debug:
        print("plist", plist)
...
if (
    resume == True
):  # This needs to be generalised to more than just a 6x3 grid, but it works for now
    past_results = np.load(previous_samples)
    pos2 = past_results.f.arr_0[-1, :, :]
    if pos2.shape != pos.shape:
        print(
            "The previously saved samples are not of the same dimensionality as those you are trying to fit for..."
        )
        print("Quitting...")
        quit()
    else:
...
if shotnoise:
    ...
    for tp in range(200):
        chisq, datacount_list, simcount_list, poisson_list = log_likelihood(
            (tempin), returnall=True, connection=tc
        )
...
if wfit:
    ...
    for i in range(npoints):
        ...
        try:
            chisq, datacount_list, simcount_list, poisson_list = log_likelihood(
                (tempin[i]), returnall=True, connection=tc
            )
        except TypeError:
            chisq = -999
        print("RESULT!", chisq, flush=True)
...
with Pool(
    nconn
) as pool:  # this is where Brodie comes in to get mc running in parallel on batch jobs.
    ...
    for qb in range(50):
        ...
        # Run the sampler
        if qb == 0:
            state2 = sampler.run_mcmc(
                pos, 100, progress=True
            )  # There used to be a semicolon here for some reason
