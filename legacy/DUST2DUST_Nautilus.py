...
npool = 16 # match this to nodes * cpus * tasks from your slurm job?

resume=True
checkpoint = 'nautilus_test.hdf5' # the hdf5 file allows runs to be resumed, but does not allow samples to be easily accessed
n_eff=10000 # generally, n_eff = 10000 will be best. 1000 could be used for a quick test. The error on log Z will be ~ 1/sqrt(n_eff)
n_like_max = 100000  # change this to 1 if you just want to retrieve and save results from previous .hdf5 checkpoint
seed = 5768
discard_exploration = False # a bit like removing burnin if True (but probably shouldn't be necessary)
...
arrdict = {'c':np.arange(-.5,.5,.01), 'x1':np.arange(-5,5,.1), 'RV':np.arange(0,8,0.1), 'EBV':np.arange(0.0,1.5,.02),   
...
def LL_Creator(inparr, simbeta, simsigint, returnall_2=False): #takes a list of arrays - eg [[data_1, sim_1],[data_2, sim_2]] and gives an LL 
    ...
    for n, i in enumerate(inparr):                        
        if n == 0: #colour                                
            ...
            datacount = i[0]                              
            simcount  = i[1]   
            # poisson = inparr[3][0]/np.sqrt(inparr[-2][0]) #commented this out to switch to errors from data to sim ones
            poisson = inparr[3][1]/np.sqrt(inparr[-2][1]) #uses RMS/sqrt(N) as error  
        elif n == 2: #lo mass MURES                       
            datacount = i[0]                              
            simcount  = i[1]                              
            # poisson = inparr[4][0]/np.sqrt(inparr[-1][0]) #commented this out to switch to errors from data to sim ones 
            poisson = inparr[4][1]/np.sqrt(inparr[-1][1]) #uses RMS/sqrt(N) as error 
        elif (n == 3): #Hi mass RMS                       
            datacount = i[0]                              
            simcount = i[1]                               
            # poisson = i[0]/np.sqrt(2*inparr[-2][0])  #commented this out to switch to errors from data to sim ones  
            poisson = i[1]/np.sqrt(2*inparr[-2][1]) #The error on the error is sigma/sqrt(2N)                       
        elif (n == 4): #low mass RMS                      
            datacount = i[0]                              
            simcount = i[1]                               
            # poisson = i[0]/np.sqrt(2*inparr[-1][0]) #commented this out to switch to errors from data to sim ones
            poisson = i[1]/np.sqrt(2*inparr[-1][1])
    ...
    print('I got a LogLike breakdown of ',LL_list,' (colour, himass res, lomass res, himass std, lomass std, beta, sigint)', flush=True)                           
    if not returnall_2:                                   
...
def make_nautilus_prior(INP_PARAMS, CLEANDICT, override, walkfactor=2):
    ...
    for p in plist:                              
        ...
        prior.add_parameter(p, dist=truncnorm((lower - mu) / sigma, (upper - mu) / sigma, loc=mu, scale=sigma))         
    return prior
...
def do_sampling(prior, npool, chainfile, n_like_max=100000, ndim=None, nlive=None, n_eff=10000,fileout=None, resume=True, seed=None,discard_exploration=False):
    if ndim==None:
    ...
    print('GO! Running sampler with ndim=',ndim, 'n_eff=', n_eff, 'n_like_max=',n_like_max, 'nlive=',nlive, 'checkpoint=',fileout,'will save chains to ',chainfile)
    with Pool(npool) as pool:
        sampler = nautilus.Sampler(prior, log_probability, n_live=nlive, n_dim=ndim, pass_dict=False, pool=pool, filepath=fileout, resume=resume, seed=seed)
        sampler.run(verbose=True, n_eff=n_eff, n_like_max=n_like_max, discard_exploration=discard_exploration)
        sys.stdout.flush()
        print('Finished!')
...
#        logging.info("# ========== BEGIN DUST2DUST ===============")
    ...
    do_sampling(prior, npool, CHAINS, n_like_max=n_like_max, n_eff=n_eff,fileout=checkpoint, resume=resume, seed=seed, discard_exploration=discard_exploration)
